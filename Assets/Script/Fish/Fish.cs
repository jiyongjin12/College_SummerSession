...
{ // 처리
    ...
    {
        HandleReDetectionCooldown(); // 재탐지 쿨다운 타이머를 여기서 감소시킵니다.
        ...
        else if (IsActingOnPlayer) // 플레이어와 상호작용 중 (추적, 응시, 공격 등)
        {
            HandlePlayerInteraction();

            // Interaction이 끝나면 ResetPlayerActionState()가 호출되고,
            // 이 안에서 IsActingOnPlayer = false; 와 IsOnReDetectionCooldown = true; 가 설정됩니다.
            _currentActionTimer -= Time.deltaTime;
            if (_currentActionTimer <= 0)
            {
                ResetPlayerActionState(); // 여기서 재탐지 쿨다운이 시작됩니다.
            }
            UpdateVelocity();
        }
        else // 일반적인 군집/방황 상태 (플레이어와 상호작용 중이 아닐 때)
        {
            if (!IsOnReDetectionCooldown) // 재탐지 쿨다운이 아닐 때만 플레이어를 감지합니다.
            {
            ...
            UpdateVelocity(); // 군집 시스템 가속도는 FishSimulationManager에서 계산되어 이 velocity에 영향을 줍니다.
        }
    ...
    protected virtual void OnDrawGizmosSelected()
    {
        if (fishData != null)
        {
            ...
            // 수정 부분: Vector3 * Vector3 대신 Vector3 * float 로 수정
